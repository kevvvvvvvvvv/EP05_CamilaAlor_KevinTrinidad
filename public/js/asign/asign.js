$(document).ready(function () {
    ...
    document.querySelector('#eliminarDato').addEventListener('click', function () {
        ...
        if (selectedRow.length) {
            ...
            const horarioasigId = rowData[0]; // Suponiendo que el ID está en la primera columna
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        error: function (xhr, status, error) {
                            Swal.fire({
                                title: "Error",
                                text: "Ha ocurrido un problema al intentar eliminar tu asignación",
                                icon: "error"
                            });
                        }
                    });
    ...
    document.querySelector('#actualizarDato').addEventListener('click', function () {
        ...
        if (selectedRow.length) {
            const rowData = selectedRow.data(); // Obtener datos de la fila seleccionada
            const horarioasigId = rowData[0]; // Suponiendo que el ID está en la primera columna
            window.location.href='asign/'+horarioasigId+'/edit';
        } else {
            const Toast = Swal.mixin({
                toast: true,
                position: "top-end",
